...
export function positionFloating(trigger, panel, placement = "top") {
  ...
  const preferred = coordinates[placement] ?? coordinates.top;
  const fallbackPlacement = { top: "bottom", bottom: "top", start: "end", end: "start" }[placement] ?? "bottom";
  ...
  const resolvedPlacement = fits ? placement : fallbackPlacement;
  const resolved = coordinates[resolvedPlacement];
